...
* 代码实现　memoryview01.py


### 双向队列和其他形式的队列
* 利用　.append 和　.pop 方法可以把列表当作栈和队列来使用。但由于列表是顺序存储结构，删除列表的第一个元素或在第一个元素前添加元素都很耗时，
因为这些操作都会牵扯到移动列表里所有的元素。

* collections.queue 类(双向队列)是一个线程安全、可以快速从两端添加或删除元素的类型。
如果想要有一种数据类型来存放最近的几个元素，deque 是一个很好的选择。
这是因为在新建一个双向队列的时候,你可以指定这个队列的大小,如果这个队列满
