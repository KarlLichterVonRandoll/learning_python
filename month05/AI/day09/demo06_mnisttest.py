...
with tf.Session() as sess:
    ...
    # saver.save(sess, 'datalab/36899/model.ckpt')
    myPrediction = sess.run(predictions,feed_dict={x:x_test,keep_prob:1.0})
    label_test = np.argmax(myPrediction,axis=1)
    pd.DataFrame(label_test).to_csv('datalab/38247/result.csv')
